...
// ============================================================
// DOWN BANNER\n// ============================================================\nfunction loadDownBanners(){\n    if(typeof db === "undefined" || !db) return;\n    db.ref("media/downBannerConfig").on("value", snapshot=>{\n        const data=snapshot.val()||{};\n        const banners=Array.isArray(data.banners)?data.banners.filter(Boolean):[];\n        const section=document.getElementById("downBannerSection");\n        const slider=document.getElementById("downBannerSlider");\n        const dots=document.getElementById("downBannerDots");\n        if(!section||!slider||!dots)return;\n        if(!banners.length){section.style.display="none";return;}\n        section.style.display="block";\n        slider.innerHTML=banners.map((url,i)=>`<div class="down-banner-slide ${i===0?"active":""}"><img src="${escapeHtml(url)}" alt="VORNEX banner ${i+1}"></div>`).join("");\n        dots.innerHTML=banners.map((_,i)=>`<span class="down-banner-dot ${i===0?"active":""}"></span>`).join("");\n        let index=0;\n        if(window.__vornexDownBannerTimer)clearInterval(window.__vornexDownBannerTimer);\n        if(banners.length>1){\n            window.__vornexDownBannerTimer=setInterval(()=>{\n                const slides=slider.querySelectorAll(".down-banner-slide");\n                const dotEls=dots.querySelectorAll(".down-banner-dot");\n                slides[index]?.classList.remove("active");dotEls[index]?.classList.remove("active");\n                index=(index+1)%slides.length;\n                slides[index]?.classList.add("active");dotEls[index]?.classList.add("active");\n            },Math.max(2,Number(data.speed)||4)*1000);\n        }\n    });\n}\nif(document.readyState==="loading")document.addEventListener("DOMContentLoaded",loadDownBanners);else loadDownBanners();\n